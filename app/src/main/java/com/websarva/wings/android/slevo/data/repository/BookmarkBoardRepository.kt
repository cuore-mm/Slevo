...
import com.websarva.wings.android.slevo.data.datasource.local.entity.bookmark.BoardWithBookmarkAndGroup
...
class BookmarkBoardRepository @Inject constructor(
     ...
     */
    suspend fun upsertBookmark(bookmark: BookmarkBoardEntity) {
        gate.withWritePermit {
     ...
     * 外側で gate を取得し、内側の `ensureBoard` は ungated helper を使う。
     * @return 登録に使用した boardId
     */
    suspend fun upsertBookmark(boardInfo: BoardInfo, groupId: Long): Long =
        gate.withWritePermit {
            val bId = boardRepository.ensureBoardUngated(boardInfo)
     ...
     */
    suspend fun deleteBookmark(boardId: Long) {
        gate.withWritePermit {
