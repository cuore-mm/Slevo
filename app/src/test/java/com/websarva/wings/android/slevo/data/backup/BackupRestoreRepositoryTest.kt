...
/**
 * [BackupRepositoryImpl] の preview / restore orchestration を検証する。
 */
...
class BackupRestoreRepositoryTest {
    ...
    fun previewBackup_returnsFailureWhenInputOpenFails() = runTest {
        ...
        assertTrue(result is BackupRestoreResult.Failure)
    }
    ...
    fun restoreBackup_createsPendingRestoreForValidZip() = runTest {
        ...
        assertTrue(result is BackupRestoreResult.Success)
        assertTrue(pendingRestoreManager.readMarker() != null)
    }
    ...
    fun restoreBackup_excludesCookiesWhenSkipRequested() = runTest {
        ...
        assertEquals(false, marker.includeCookies)
    }

    @Test
    fun restoreBackup_returnsFailureWhenManagerFails() = runTest {
        // 既存 prepared marker を作成
        val pendingDir = File(context.filesDir, PendingRestoreManager.PENDING_DIR_NAME)
        File(pendingDir, PendingRestoreManager.MARKER_FILENAME).apply {
            parentFile?.mkdirs()
            writeText(moshi.adapter(PendingRestoreMarker::class.java).toJson(
    ...
    private fun buildZip(block: ZipOutputStream.() -> Unit): java.io.InputStream {
    ...
    private fun ZipOutputStream.writeManifestEntry(cookies: Boolean) {
        val manifest = BackupManifest(
            ...
            backupMode = "full",
            included = IncludedContents(database = true, settings = true, tabs = true, cookies = cookies),
        )
    ...
    private fun ZipOutputStream.writeSettingsEntry() {
        val settings = BackupSettingsJson(
            ...
            gestureSettings = BackupGestureSettings(enabled = false, showActionHints = true, actions = emptyMap()),
        )
    ...
    private class FakeSettingsDataSource : SettingsLocalDataSource {
        ...
        override fun observeGestureSettings(): Flow<GestureSettings> = flowOf(GestureSettings.DEFAULT)
        override suspend fun setGestureEnabled(enabled: Boolean) {}
        override suspend fun setGestureShowActionHints(show: Boolean) {}
        override suspend fun setGestureAction(direction: com.websarva.wings.android.slevo.data.model.GestureDirection, action: com.websarva.wings.android.slevo.data.model.GestureAction?) {}
    ...
    private class FakeCookieDataSource : CookieLocalDataSource {
        override fun getCookies(): Flow<List<okhttp3.Cookie>> = flowOf(emptyList())
        override suspend fun saveCookies(cookies: List<okhttp3.Cookie>) {}
    }
}
