...
class BackupReaderTest {
    ...
    @Test
    fun readBackup_dbVersionMismatch_returnsInvalid() {
        val zip = createValidZipBytes(
            manifest = createValidManifest(databaseVersion = 8),
        )
        val error = readError(zip)
        assertTrue(error is BackupRestoreResult.Invalid)
        assertTrue((error as BackupRestoreResult.Invalid).detail.contains("databaseVersion"))
    }
    ...
    fun readBackup_schemaValidationFails_returnsInvalid() {
        ...
        val error = readError(
            zip,
            dbValidator = FakeBackupDatabaseValidator(validationError = "missing table: foo"),
        )
        assertTrue(error is BackupRestoreResult.Invalid)
 ...
 *
 * [validationError] を設定するとそのエラーを返す。null の場合は常に成功する。
 */
