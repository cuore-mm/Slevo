...
#### Scenario: 取得側が除去する先頭空白を同一視する
- **WHEN** 投稿入力の本文が先頭の空白または改行で始まり、取得レスの本文では取得時のtrim処理によってそれらが除去されている
- **THEN** システムは取得側の本文境界と同じ正規化を投稿入力にも適用し、本文を一致候補として扱う
