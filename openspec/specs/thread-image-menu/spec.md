...
システムは `ImageMenu` の BottomSheet に表示する「レス内の画像をすべて保存」項目に、`download` をベースとして右下に `filter_none` バッジを重ねた合成アイコンを表示しなければならないMUST。合成描画はベースアイコンとバッジアイコンを引数で指定できる共通実装で行わなければならないMUST。重なり部分では Clear 合成で切り抜きを行い、ベースアイコンが透けて見えない表示を維持しなければならないMUST。この要件は単体保存との視覚的識別を目的とし、既存の一括保存実行条件および保存動作契約を変更してはならないMUST。
