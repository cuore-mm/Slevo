# resolve-url-routing Specification

## Purpose
TBD - created by archiving change unify-url-routing. Update Purpose after archive.
## Requirements
### Requirement: 共通URLリゾルバの提供
システムはURL文字列を解析し、板/スレの種別と必要な識別子（host/boardKey/threadKey）を返す共通リゾルバを提供することを SHALL 要求する。

#### Scenario: 解析結果を構造化して返す
- **WHEN** URLリゾルバがURLを解析する
- **THEN** 種別（Board/ItestBoard/Thread/Unknown）と `host` / `boardKey` / `threadKey` を含む結果を返す
