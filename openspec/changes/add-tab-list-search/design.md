...
### Decision 3: 検索フィルタは純粋関数として切り出し、既存検索ロジックを再利用する

検索対象文字列とクエリの比較には、既存のスレ内検索・板内検索と同じ `toHiragana()` と `contains(..., ignoreCase = true)` の組み合わせを採用する。これを `matchesSearchQuery(content, query)` の純粋関数として共通化し、タブ一覧検索でも再利用する。板タブは板名と表示サービス名、スレッドタブはスレ名と板名を検索対象にする。
