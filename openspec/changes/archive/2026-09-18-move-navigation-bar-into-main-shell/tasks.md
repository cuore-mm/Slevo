...
- [x] 1.2 default付き`AppRoute.MainShell`、`MainShellStartDestination`、`BbsEntryTransition`、`MainShellBbsOrigin`を追加し、新しい型へ規約どおりKDocとenum用`@Keep` / `@Serializable`を付ける。MainShellにはbase/contextual識別用のmodeを保存しない
...
- [x] 4.5 Board / Threadのタブ一覧操作を`AppRoute.MainShell(startDestination = Tabs)`のRoot pushへ変更し、直前Root entryだけをsourceとして扱う
