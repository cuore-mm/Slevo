...
- 主な対象は`AppScaffold.kt`、`AppRoute.kt`、`RootNavGraph.kt`、`MainShellNavGraph.kt`、`NavigationExtensions.kt`、`TransitionSpecs.kt`、`NavigationBottomBar.kt`、`TabsScaffold.kt`、`BbsRouteScaffold.kt`、`RegisteredBBSNavigation.kt`、`SettingsRoute.kt`、Deep linkおよびRoot/MainShell間Navigationの呼び出し元である。MainShell内の下部バー切替用`RenderBottomBar.kt`は削除する。
