...
`previousBackStackEntry`から復元した`sourceRoute`をcontextual Tabsの判定と履歴統合へ使用する。Board / Threadの既存ジェスチャー・
オーバーフローメニューからBookmarkまたはBBS一覧を開く場合は、対応する`MainShellStartDestination`を保存して対象inner画面を初期表示する。
...
Board / ThreadからTabsを開くと、Rootへ`AppRoute.MainShell(startDestination = Tabs)`をpushし、そのentryのMainShellNavHostをTabsから開始する。
Root MainShell entryの直前がBoard / Threadで、inner Tabs entry IDが現在entryと一致するときだけ、その`sourceRoute`をcontextual Tabsの
履歴処理へ使用する。Tabs選択要求はRoot MainShell entry ID、inner Tabs entry ID、選択元`MainShellBbsOrigin`を持つ。

```kotlin
enum class MainShellBbsOrigin { Tabs, Bookmark, BbsServiceGroup }
```

Root stackが`X → Board A → MainShell(startDestination = Tabs)`で、直前entryから復元した`sourceRoute`がBoard A、現在inner entryがTabs、
かつ`origin == Tabs`なら、Board Bへのnavigateで`popUpTo(Board A, inclusive = true)`を指定し、最終stackを`X → Board B`にする。
Thread同種および別種選択は既存`NavigationExtensions.kt`の最終stack規則を同じRoot stack上で再現する。

contextual Tabs内でBookmarkへ移動した場合、inner stackを`Tabs → Bookmark`として保持する。BookmarkからBoard Bを開く要求は`origin == Bookmark`なので通常pushし、Root=`X → Board A → MainShell(startDestination = Tabs) → Board B`を維持する。BackはBoard BをpopしてBookmarkを表示し、次のBackでinner BookmarkをpopしてTabs、さらにBackでMainShell entryをpopしてBoard Aへ戻る。
...
### 8. MainShellの履歴文脈をRoot back stackから導出する

MainShell routeへbase/contextualを表すmodeは保存しない。RootのMainShell entryが生成された直後に、直前の可視entryがBoardまたはThreadで、
そのMainShellの`startDestination == Tabs`なら`sourceRoute`を導出する。この`sourceRoute`が存在する場合だけ、そのMainShellをcontextual Tabsとして
扱い、Tabsからの直接選択でRoot履歴統合を行う。Root startのMainShellは直前entryがないためsourceRouteを持たず、Bookmark / BBS一覧を初期表示する
MainShellもTabsの直接選択文脈を持たない。

MainShellのNavigationBarは既存の`launchSingleTop`、`saveState`、`restoreState`を維持する。TabsからBookmark / BBSサービスへの移動はinner back stackへ
積み、BackでTabsへ戻れるようにする。これによりユーザー確定済みの`X → Board → Tabs → Bookmark → Board`を畳み込まない。

NavigationBarの選択表示は各MainShell entryの現在inner destinationから導出し、別のMainShell entryのcontrollerを直接操作しない。
...
- `BbsEntryTransition`には`@Keep`と`@Serializable`を付け、既存Board / Thread route引数へdefault値を追加する。MainShellのroute引数は
  `MainShellStartDestination`だけとし、base/contextualを表すmodeを追加しない。
...
- Rootの各MainShell entryは別々の`NavHostController`を所有し、一つのcontrollerを複数NavHostへ同時接続しない。contextual Tabsの判定はentryの
  `startDestination`と直前Root entryから導出する。
...
- プロセス再生成でMainShell inner back stackが復元できない場合、直前Root entryがBoard / Threadで`startDestination == Tabs`のときだけsourceRouteを
  導出し、それ以外は保存済みRoot entryの初期routeへフォールバックする。
...
- 実`NavHostController`を使い、Root startのMainShellでTabs / Bookmark / BBSサービスを切り替えてもRoot stackが増えず、saveState / restoreStateされることを確認する。
...
- Activity再生成と可能ならprocess recreation testでRoot直前entryからの`sourceRoute`、各MainShell inner stack、`BbsEntryTransition`、Root Snackbar候補を復元する。
...
- [Root transition中に同じMainShell controllerが二つのNavHostへ接続される] → Root entryごとにcontrollerを生成し、各entryのinner stateを分離する。
...
- [route引数の変更で保存済みback stackを復元できない] → `MainShellStartDestination`と`BbsEntryTransition`のdefault値、`@Keep`、再生成testを先に追加し、互換性を確認してから全呼び出し元を移行する。
...
5. 直前Root entryからのsourceRoute導出、直接Tabs選択のstack統合、Bookmark / BBS経由の履歴保持、entry ID guardを実装する。
