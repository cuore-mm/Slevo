...
### 1) ドラッグは絶対位置マップ制御を採用する
- 決定: `onVerticalDrag` は相対移動量をそのまま使わず、指の Y 位置から目標スクロール位置を算出して `scrollBy` で差分適用する。
- 理由: 相対制御では倍率揺れやコルーチン遅延による速度ムラが残るため、絶対位置で安定化する。
- 代替案:
...
### 3) インジケーター位置は連続値で算出する
- 決定: `firstVisibleItemIndex` と `firstVisibleItemScrollOffset` から得た連続位置を、ドラッグと同じ座標系に投影して描画する。
