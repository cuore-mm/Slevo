...
- `BoardScreen` との共通化は本変更スコープに含めず、`ThreadScreen` 周辺のみを対象にする。
...
- Affected code:
  ...
  - 必要に応じて `app/src/main/java/com/websarva/wings/android/slevo/ui/thread/screen/` 配下の `effects` / `components` サブパッケージ
- User impact:
