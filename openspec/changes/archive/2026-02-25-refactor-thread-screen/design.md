...
- `BoardScreen` との共通化や、他画面へ波及する共通化リファクタリング。
...
- **ファイル配置は `ThreadScreen` 周辺に閉じる**
  - 分割先は `ui/thread/screen` 配下を基本とし、必要に応じて `effects` / `components` サブパッケージを導入して責務単位で配置する。
  - `BoardScreen` 側との共通化を前提にした配置変更は本変更では行わない。
