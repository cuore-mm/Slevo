...
リストのスクロール状態は現在 `RemovableTabList` の内部 `rememberLazyListState()` に閉じている。検索クエリ入力によって完全リストから検索結果リストへ切り替わる直前の位置へ戻すには、板一覧とスレッド一覧それぞれの `LazyListState` を上位で保持できるようにする必要がある。
...
- タブ一覧上部にも Haze を適用し、下部操作群と視覚表現および背面リストへのタップブロック挙動を揃える。
- `SearchBottomBar` の検索入力部分を共通化し、既存の板・スレッド画面検索 UI の機能を維持する。
- 大文字小文字、ひらがな/カタカナを区別しないタブ検索を提供する。
- 検索クエリ入力で完全リストから検索結果リストへ切り替わる直前のスクロール位置を保存し、検索クエリクリアまたは検索終了で完全リストへ戻るときに復元する。
...
`RemovableTabList` に `listState: LazyListState = rememberLazyListState()` を追加し、既存呼び出し元の互換性を保つ。タブ一覧では板一覧用とスレッド一覧用の `LazyListState` を `TabScreenContent` で保持する。検索モード開始時点ではまだ完全リストを表示しているためスクロール位置を保存しない。検索クエリが空から非空へ切り替わり、完全リストから検索結果リストへ変わる直前に、両方の `firstVisibleItemIndex` と `firstVisibleItemScrollOffset` を保存する。検索クエリクリア、または検索モード終了によって完全リストへ戻るときは、完全リストが再描画された後に保存値へ `scrollToItem` で復元する。
...
### Decision 6: 上部 Haze 領域は下部操作群と同じタップガードで背面リストへの操作を防ぐ

上部 Haze 領域は固定高さでリストの前面に重なるため、下部操作群と同じくタップを消費して背面リストのカード操作を発生させない。`TabListTopSearchArea` の Haze コンテナ全体に視覚フィードバックなしのタップガードを適用し、検索ボタンや検索バーなど領域内の子要素操作は維持する。タップガードは余白を含む上部 Haze 領域全体を対象にするため、コンテンツ用 padding より前に適用する。
...
- [Risk] 検索クエリ入力でリストが切り替わった後にスクロール位置を保存すると、検索結果リスト側の位置を保存してしまう。 → 検索クエリが空から非空へ切り替わる直前の完全リスト位置を保存し、完全リストへ戻った後に復元する。
- [Risk] 検索解除時に保存した index が現在のリスト件数を超える。 → 復元時は対象リストの最終 index 以内へ丸め、空リストではスクロール復元をスキップする。
- [Risk] 上部 Haze 領域がタブカードのタップを透過する。 → 下部操作群と同様に上部 Haze 領域全体でタップを消費し、検索ボタンや検索バーの子要素操作は維持する。
