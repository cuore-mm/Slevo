...
- [x] 1.1 `TabScreenContent` 内の URL 入力処理分岐を整理し、板 URL / スレ URL / itest 板 URL の各非同期処理で検証終了処理が必ず実行されるようにする
- [x] 1.2 URL 入力処理の結果を表す typed result（板遷移、スレ遷移、エラー）を定義する
- [x] 1.3 URL 判定、ホスト解決、route 正規化を `TabsViewModel` または専用 handler へ移し、Composable から長い inline 分岐を取り除く
- [x] 1.4 `TabScreenContent` は typed result に基づいて既存の `navigateToBoard` / `navigateToThread` と dialog / drawer close を実行する

## 2. BottomSheet と長押し overlay の分割

- [x] 2.1 `TabDetailBottomSheets` を切り出し、`detailBoardTab` / `detailThreadTab` と dismiss callback から BottomSheet を描画する
- [x] 2.2 `TabLongPressOverlayLayer` を切り出し、dim overlay、floating card、action menu、BackHandler をまとめる
- [x] 2.3 floating card の enter / exit アニメーション仕様、scale 値、duration、表示条件を分割前と同じに保つ
- [x] 2.4 board/thread floating card の共通配置処理を整理し、座標変換と `graphicsLayer` scale 適用の重複を減らす
