...
- [ ] 3.1 スクロール位置保存 Composable を単体で表示するテスト用ホストを作成する
- [ ] 3.2 `isActive` が true から false へ変わったとき、現在のスクロール位置が保存されることをテストする
- [ ] 3.3 テスト用ホストを composition から外したとき、現在のスクロール位置が保存されることをテストする
- [ ] 3.4 同一位置が非アクティブ化保存と破棄時保存で重複保存されないことをテストする
