...
- [ ] 1.3 `MainActivity`、`AppScaffold`、`AppNavGraph` で `TabsViewModel` を渡している経路を図示し、`TabSessionStore` または必要な操作ラムダへ置き換える単位を決める

## 2. セッション状態取得経路の置き換え

- [ ] 2.1 `MainActivity` から `TabsViewModel` を生成・保持する処理を削除し、`TabSessionStore` を上位から利用できる構造へ変更する
- [ ] 2.2 `AppScaffold` と `AppNavGraph` の引数を `TabsViewModel` から `TabSessionStore` または必要な状態・操作へ置き換える
...
- [ ] 2.6 `TabsScaffold` と `TabsBottomSheet` が `TabSessionStore` と `TabListViewModel` を明示的に接続するように変更する
...
- [ ] 3.3 `navigateToBoard` / `navigateToThread` などの navigation helper が `TabsViewModel?` を受け取らないようにし、`TabSessionStore` または正規化・タブ確保用ラムダを受け取るように変更する
- [ ] 3.4 Deep Link、履歴、ブックマーク、BBSルート、板画面、スレッド画面からの遷移で、タブ確保とルート正規化が従来通り実行されるように接続する

## 4. URL検証状態の分離

- [ ] 4.1 `BbsRouteScaffold` の URL入力ダイアログ検証中状態を `TabsUiState.isUrlValidating` から画面ローカル状態または専用 ViewModel 状態へ移す
