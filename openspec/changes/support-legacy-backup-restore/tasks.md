...
- [x] 5.5 rollback-required marker/result 書き込み失敗時処理を実装する。完了条件: rollback-required result 書き込み失敗時は marker を変更せず、marker atomic replace 失敗時は migration-pending marker を source of truth として保持し、どちらの場合も live DB file を変更せず rollback backup を残し、completion checker から例外を外へ投げない。
...
- [x] 5.9 completion checker の unit tests を追加する。完了条件: marker なし、migration-pending success、post validation failure → rollback-required、completed marker write failure、completed marker 後の success result write failure、rollback backup deletion failure、staging cleanup failure、marker deletion failure の各動作が検証される。
- [x] 5.10 rollback-required 記録失敗の unit tests を追加する。完了条件: rollback-required marker write failure、rollback-required result write failure、completion checker exception が外へ出ないこと、migration-pending と rollback backup が保持されることを検証する。
