...
### 2) UI 適用は段階的に行う
- **Decision**: 第1段階で `ImageThumbnailGrid` に進捗表示を導入し、その後に `ImageViewerPager` へ同等契約を適用する。
...
4. 既存機能（共有トランジション、メニュー、画像切替）の非回帰確認を行う。

## Open Questions

- issue 444 の「画像サムネイル」に、画像ビューア下部のサムネイルバーを含めるか（`ImageViewerThumbnailBar` まで対象化するか）。
