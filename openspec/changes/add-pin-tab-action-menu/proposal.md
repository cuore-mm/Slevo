...
- 長押し選択中は下部操作群のボタンを非表示にし、ページ切替インジケータと更新進捗インジケータは表示を維持する。
...
- タブ固定状態を永続化し、固定済みタブではカード右上の閉じるアイコンを固定アイコンに置き換える。
- 固定済みタブは一覧上で通常タブより上に表示し、固定タブ同士・通常タブ同士の相対順は既存のタブ順に従う。
...
- `tablist-ui`: タブ一覧カードの長押しアクションメニュー、固定状態表示、詳細表示、選択中の下部操作群表示を追加する。
