...
`AppRoute.Thread.threadTitle` は表示用の名称でありURLではないため、5ch.net→5ch.ioのURL正規化対象に含めない。URL入力、Deep Link、レス本文リンクのようにタイトル未取得の入口では `threadTitle` を未設定として扱い、スレ読み込み後に実タイトルで更新する。仮タイトルとして元URLを入れる方式は、タブ名や画面表示に旧hostが残るため採用しない。
...
既存の `5ch.net` タブについては、設定オンで開いたタイミングから正規化後routeでタブを保証する。重複タブを避けるため、タブ照合では `5ch.net` と `5ch.io` の同一板/同一スレを同じ対象として扱うか、既存タブを正規化後routeへ置き換える処理を明確にする。保存データ全体を書き換える移行方式は、ロールバックが難しく、ユーザーの既存データを不可逆に変更するため採用しない。
