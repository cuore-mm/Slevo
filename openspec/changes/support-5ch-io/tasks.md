...
- [ ] 5.4 CIビルド/ユニットテストを実行し、失敗があれば修正する

## 6. route正規化とタブ保存の再整理

- [ ] 6.1 `navigateToBoard` / `navigateToThread` の共通入口でrouteを正規化し、正規化後routeをタブ保証と画面遷移の両方に使う
- [ ] 6.2 `AppRoute.Thread.threadTitle` をURL正規化対象から外し、タイトル未取得のURL由来入口では未設定として扱えるようにする
- [ ] 6.3 URL入力、Deep Link、レス本文リンクなど、タイトル未取得の入口が元URLをタブ名として保存しないことを確認する
- [ ] 6.4 設定オン時に `5ch.net` の既存タブを開く場合、重複タブを作らず `5ch.io` のタブ保存routeへそろえる方針を実装する
- [ ] 6.5 ブックマーク、履歴、板DBは一括移行せず、そこから開いたタブだけが `5ch.io` routeで保存されることを確認する
- [ ] 6.6 route正規化、タブ保存、タイトル未設定時の表示fallbackに関する単体テストまたは既存テスト更新を追加する
