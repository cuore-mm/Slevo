...
現在の `TabsViewModel` は、アプリ全体で共有するタブセッション管理と、タブ一覧画面だけで必要な検索・選択・BottomSheet などの一時 UI 状態を同時に保持している。これにより、タブ一覧画面から離れた後も画面固有状態が残りやすく、画面ライフサイクルに沿った状態初期化や責務分離が分かりにくい。

## What Changes

- タブ一覧画面専用の ViewModel を導入し、検索状態、長押し選択状態、削除中状態、詳細 BottomSheet 表示状態などの画面固有 UI 状態を画面スコープで管理する。
- 既存の `TabsViewModel` は、開いている板/スレッドタブ、タブ永続化、ページ状態、スレッドタブ更新、子 ViewModel キャッシュなど、アプリ内タブセッション管理に責務を寄せる。
- タブ一覧画面は、セッション管理 ViewModel の状態を参照しつつ、画面専用 ViewModel がフィルタリングや一時 UI 操作を組み立てる構成へ移行する。
...
- `tablist-ui`: タブ一覧画面の画面固有 UI 状態を画面ライフサイクルに紐付け、既存の表示・操作仕様を維持する責務分離要件を追加する。
...
- 影響範囲:
  ...
  - ナビゲーション/Activity からの ViewModel 受け渡し箇所
