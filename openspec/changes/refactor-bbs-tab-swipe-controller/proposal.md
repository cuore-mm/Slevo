...
- タイトルカードを本文と同じ Pager offset から描画し、本文とタイトルそれぞれのviewportにおける表示進行率を揃える。ブックマーク、タイトル、更新、カード下端のロード進捗を本文と同期して移動させ、タイトル専用の PagerState は作成しない。タブ固有の状態購読とカード描画はstable key単位で管理し、描画window移動時に別タブの状態を再利用しない。
- 固定Toolbarの展開時はタイトル行と下段アクション群が同時に表示できる高さを確保し、タイトルカードと画面種別ボタンの高さを揃える。縮退時の56dpは維持する。
