...
板更新時の差分計算で「更新開始時点の未アーカイブ summary ID − 最新 subject.txt の ID」を削除対象とする。削除対象は `DELETE FROM thread_summaries WHERE boardId = :boardId AND threadId IN (:threadIds)` のような板内 ID 条件で削除する。
...
### 3. 既存アーカイブ済み summary は移行または起動時 cleanup で削除する

既存 DB には `isArchived = 1` の行が残っている可能性がある。DB バージョンを上げる場合は migration で削除し、スキーマ変更を伴わない場合は起動時または板更新前後の cleanup で削除する。

どちらの方法でも削除対象は `isArchived = 1` に限定し、`isArchived = 0` の現役 summary は削除しない。大量行がある可能性を考慮し、通常処理の cleanup は上限件数を持たせる。migration では一括削除の可否を DB サイズと実行時間の観点で確認する。
...
- [Risk] 既存の `isArchived = 1` 行が多く、migration または cleanup に時間がかかる。 → 削除方式を選定し、通常処理では上限件数またはチャンク分割を使う。
...
3. 既存 `isArchived = 1` 行を削除する migration または cleanup を追加する。
4. 大量削除、通常更新、新規追加、再出現スレッド、履歴・ブックマーク保持のテストを追加する。
5. 問題がなければ将来の別変更で `isArchived` カラム削除を検討する。

ロールバック時は、削除済み summary を復元できない。ただし履歴・ブックマーク・タブ・共通客観状態は独立して保持されるため、ユーザーの主要データは維持される。

## Open Questions

- 既存 `isArchived = 1` 行の削除は migration で一括実行するか、起動時 cleanup で分割実行するか。
