...
- **状態ホルダーを導入して Screen ローカル state を集約する**
  - `isBarsVisible` などの画面内 state をまとめて保持し、下位コンポーネントへ必要な値とイベントだけを渡す。
  - 代替案として state を個別に散在させる構成は、責務境界が不明確なまま残るため採用しない。
...
- [Risk] state の受け渡し経路が増え、初期分割時にパラメータ不整合が発生する可能性。
  - Mitigation: 状態ホルダー経由で公開項目を最小化し、責務ごとの入出力を明示する。
