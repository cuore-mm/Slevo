...
2. `SlevoTabDragGestureDetector`のpost-long-press区間をMain pass所有ループへ置き換え、横スワイプDetectorを常設pointer nodeの内部ゲートへ変更し、drag中無効化とoffset復帰を追加する。閾値超過時はhandoff offsetをカードへ渡し、抵抗位置から指位置までを描画アニメーションする。
