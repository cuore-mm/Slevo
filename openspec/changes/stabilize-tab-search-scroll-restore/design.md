...
### Decision 5d: 検索表示エリア内でも結果あり/なしをフェードで切り替える

通常リストと検索結果リストの切り替えにはすでにフェードを使っているが、検索結果表示中にフィルタ結果が 1 件以上から 0 件へ、または 0 件から 1 件以上へ変わるときは、現状では検索表示エリアの中身が即時に差し替わる。

このため、検索表示エリア内にも小さな `AnimatedContent` を追加し、検索結果リストと空状態メッセージを短いフェードで切り替える。通常リストとの切り替えとは別レイヤーにすることで、既存の通常/検索フェードを保ったまま、検索中の視覚的な違和感だけを減らす。
...
- [Risk] 検索中の結果あり/なし差し替えが急で違和感がある → 検索表示エリア内でも短いフェードを適用する。
