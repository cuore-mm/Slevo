...
- `TabListUiState`: 検索結果リストの先頭表示要求に関する UI 状態を公開する。通常リスト復元待ち状態は不要にする。
- `TabScreenContent`: 通常用・検索用の `LazyListState` を別々に持ち、検索クエリの有無で表示するリストと state を切り替える。検索解除時の復元スクロール副作用は削除する。
- `TabsBottomSheet` / `BbsRouteScaffold`: BottomSheet dismiss 時に検索状態を閉じる導線を追加する。
- テスト: ViewModel のクエリ遷移・検索結果先頭表示要求/消費、および通常リストと検索結果リストのスクロール状態が独立することを検証する。
