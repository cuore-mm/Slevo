...
スワイプ削除の確定判定は、カード幅に対する移動距離だけでなく、`VelocityTracker` が算出する右から左方向の速度も考慮する。距離判定はカード幅の 35% をしきい値とし、速度判定は 700dp/s 相当を `LocalDensity` で px/s に変換した値をしきい値とする。速度だけで微小な揺れが削除として扱われないよう、速度判定には 20dp 相当以上の左方向移動も必要とする。
