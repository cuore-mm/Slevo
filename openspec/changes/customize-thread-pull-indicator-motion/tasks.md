...
- [ ] 2.2 閾値到達で最大サイズとなり、閾値未満へ戻した場合に再縮小するクランプ処理を実装する
- [ ] 2.3 `delta` を使わず `overscroll` 進捗由来の角度式で、プル中は左回転・戻し中は右回転に見えるロジックを適用する

## 3. 回帰確認

- [ ] 3.1 閾値未満・閾値到達・閾値超過・閾値超過後の戻し操作・更新中の表示挙動を手動確認する
