...
pin toggle intent は enqueue 前に要求値を確定せず、`threadId` と completion を運ぶ action とする。worker は先行 intent の DB write、Flow confirmation、pending cleanup が完了した後の投影状態から対象タブを取得し、その時点の `isPinned` を反転して `Pin` pending operation と repository の要求値を生成する。対象タブがその時点で存在しない場合は対象なしの no-op として完了する。これにより、最初の pending 登録前に複数 toggle が到着しても各 toggle は先行操作の結果へ順番に作用し、偶数回で元の値、奇数回で反転値へ決定的に収束する。
...
pin mutation では、cancellable な repository 呼出しが成功を返した事実を coordinator が観測可能な commit 完了境界とする。それ以前は caller cancellation が intent operation と repository 待機へ伝播し、既存の rollback 契約に従って pending を除去できる。成功結果は caller-owned child のローカルな `AtomicReference` 相当へ suspend point を挟まず発行し、FIFO worker は child の終了後にその結果の有無だけで write phase の結果を判定する。caller cancellation callback は成功結果が未発行の間だけ child を cancel できる。callback が結果発行直前に cancellation を先に記録しても、repository の正常返却を同じ同期結果へ発行できた場合は commit 済み結果が常に優先され、callback は結果を消去せず、pending cleanup も行わない。

commit 済み結果がある場合、FIFO worker は caller-owned child の cancellation 状態ではなく結果を引き取り、worker-owned reconciliation phase として matching Flow を待つ。したがって repository suspend call の返却後に別の ownership CAS を行う構造は使用しない。後者は caller completion の cancellation を確認条件に使用せず、`Pin` pending projection を保持したまま baseline より新しく期待 pin 値と一致する canonical Flow を待ち、その確認後にだけ pending を除去して FIFO を進める。caller completion が既に cancel 済みなら成功を再通知せず、caller 固有の後続処理も再開しない。repository が正常結果を発行せず cancellation または失敗で終了した場合だけ caller-owned cleanup を行う。coordinator scope cancellation は FIFO worker と reconciliation を従来どおり cancel する。この phase 遷移により、commit 後かつ Flow 前の cancellation で古い canonical 値から次 toggle を導出せず、補償 write も行わない。
...
- `ThreadTabsCoordinatorTest.kt` に制御可能な Room-like Flow、`StandardTestDispatcher`、`CompletableDeferred` barrier を導入する。
  ...
  - pin repository test double が commit 成功を記録した直後、正常結果を coordinator へ返す境界内で caller を同期的に cancel し、旧実装の post-return ownership CAS なら cancellation が勝つ順序を決定的に作ること。それでも成功結果が worker-owned reconciliation へ一度だけ引き渡され、pending pin が古い canonical snapshot に再適用され、後続 toggle の write が matching Flow 前に開始しないこと。matching Flow 後に後続 toggle が確定済み pin 値を反転し、2 回の toggle で元の値へ戻ること。
...
13. pin repository 呼出しが正常結果を発行する前の cancellation は従来どおり caller-owned write phase へ伝播する。pin write child は repository 正常返却と成功結果の発行の間に suspend point を置かず、caller cancellation callback と共有する原子的結果では正常返却済み結果を cancellation より優先する。FIFO worker は child 終了後、成功結果がある場合だけ Flow reconciliation を caller cancellation link から分離して所有し、成功結果がない cancellation/failure の場合だけ caller-owned cleanup を採用する。repository suspend call の後で ownership を奪い合う独立 CAS は置かない。reconciliation は coordinator scope の cancellation には引き続き従う。cancel 済み caller へ continuation を返さず、pending `Pin` を matching canonical confirmation 前に除去せず、後続 intent を開始しない。変更は pin path に限定し、ensure metadata、delete の last-tab 補正、repository/DAO/schema を変更しない。
...
- [pin commit と post-return ownership CAS の間で caller cancellation が勝つ] → repository 正常結果を caller-owned child の原子的結果へ suspend point なしで発行し、発行と競合した cancellation より commit 済み結果を優先する。FIFO worker が結果を引き取って matching canonical revision まで pending projection と FIFO barrier を保持するため、cancel callback は commit 済み pending を cleanup できない。Flow が停止すれば後続 intent も待つが、DB 正本と toggle 順序を優先し、coordinator scope teardown では既存どおり worker と pending を破棄する。
