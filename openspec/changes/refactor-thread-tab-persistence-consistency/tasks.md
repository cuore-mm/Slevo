...
- [ ] 7.2 `app/src/main/java/com/websarva/wings/android/slevo/ui/tabs/coordinator/ThreadTabsCoordinator.kt` の pin path だけを、repository 成功返却前の caller-owned cancellable write phase と、成功返却後の worker-owned reconciliation phase に分ける。後者では caller completion cancellation を Flow 待機の中断条件にせず、matching canonical revision まで pending `Pin` と FIFO barrier を保持し、確認後に pending を一度だけ除去する。cancel 済み completion を成功再開せず、pre-commit cancellation、coordinator teardown、metadata merge、last-tab delete、他 mutation、repository/DAO/schema、UI を変更せず、補償 write を追加しない。
